...
entry = tk.Entry(root, textvariable=text_input, font=('Arial', 20), bd=10, insertwidth=4, width=20, justify='right', bg="#FFFFFF", fg="black")
...
for btn_text, command in buttons:
    if btn_text.isdigit() or btn_text == ".":
      ...
      bg_color = "#4CAF50"  
      fg_color = "#FFFFFF"
    tk.Button(root, text=btn_text, font=('Arial', 8), padx=10, pady=10, command=command, bg=bg_color, fg=fg_color, bd=0, width=button_width, height=button_height).grid(row=row, column=col, padx=5, pady=5)
    col += 1
    if col > 7:
        col = 0
        row += 1
